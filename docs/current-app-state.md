...
`AppRootView` chooses a compact `TabView` shell or a regular-width `NavigationSplitView` shell from the horizontal size class. Compact width keeps Timeline, Things, and Carry Forward as tabs and opens Settings, ledger search, and the review queue as sheets from toolbar buttons. Regular width exposes Timeline, Things, Carry Forward, Search, Review, and Settings in the sidebar, with Review omitted unless at least one `LedgerReviewItem` is in an ambiently visible state. `AppRootView` injects `AppSessionState` and `DeveloperModeState`. On launch and whenever the scene becomes active, it runs `ExtractionRecoveryMaintenanceService.repairInterruptedEntries()`, `DerivedFieldMaintenanceService.repairAll()`, and `LedgerReviewItemGenerationService.refresh()`. If this maintenance fails, the app shows an alert that cached ledger fields could not be refreshed.
...
The Things surface lists `Thing` records, supports local Thing search, and opens detail screens. Compact width uses navigation pushes; regular width uses `ThingsSplitView` with list-detail selection repair. Thing rows use `ThingPreviewSnapshot` for latest event details, reminder state, upcoming reminder dates, metadata highlights, note snippets, aliases, record counts, and review-item pills when values exist. Thing detail uses `ThingDetailSnapshot` for operational status, reminder summaries, recent activity, event chronology, notes, identity details, timeline entry points, and manual add/edit flows for events, reminders, notes, and the thing itself. Source metadata and extraction identifiers are shown only when developer diagnostics are unlocked.

The Carry Forward surface lists `LedgerRule` records in Now, Coming Up, Review, and Paused lanes from `ReminderContinuityPresentationService`. Compact width uses navigation pushes; regular width uses `RulesSplitView` with list-detail selection repair. Reminder detail supports edit, reschedule, end-date, dismiss, and delete actions according to `ReminderDetailActionPolicy`. Reminder status is derived by `RuleStatusService`; the persisted model remains `LedgerRule`.

Settings contains AI service token status, local JSON export, and local data clearing. Developer diagnostics include extraction attempt lists for all attempts, failed attempts, and the Internal QA Lab behind `DebugAccessPolicy.allowsExtractionDebugScreens` or `DebugAccessPolicy.allowsInternalQAScreens`. Developer mode is available in Debug builds, `INTERNAL_DIAGNOSTICS` builds, or UI testing when `-enable-developer-mode` is passed. Automation can force it unavailable with `-disable-developer-mode`. Developer mode is unlocked from the Settings version footer long press, from the version footer tap in automation, or by automation with `-unlock-developer-mode`, and can be locked again in Settings.
...
Search is local substring matching. `SearchService.activeMode` is `localSubstring`, and search records are built from things, events, reminders, notes, user chat messages, and timeline slices. The root search entry or regular-width sidebar opens `UnifiedSearchView`, and the Things surface has a local Thing search field.
...
`AppRuntimeConfiguration` supports UI-test, screenshot, and diagnostic launch arguments: `-ui-testing`, `-screenshot-mode`, `-use-fake-extractor`, `-reset-store`, `-reset-device-token`, `-skip-launch-maintenance`, `-use-in-memory-store`, `-enable-developer-mode`, `-disable-developer-mode`, `-unlock-developer-mode`, `-simulate-ai-service-error=...`, `-fixed-now=...`, `-ai-service-base-url=...`, `-seed-scenario=<id>`, `-initial-tab=...`, `-screenshot-seed=...`, `-screenshot-start=...`, `-screenshot-search-query=...`, `-screenshot-locale=...`, `-screenshot-time-zone=...`, `-screenshot-calendar=...`, `-screenshot-appearance=...`, and `-disable-animations`.

Legacy double-dash automation aliases and backend device-token auto-enrollment are no longer supported. Automation uses isolated UserDefaults and a deterministic store path under Application Support unless in-memory storage is requested. `-use-fake-extractor` selects `DeterministicMessageExtractionClient`, deterministic screenshot mode also disables animations, and `-simulate-ai-service-error=...` is honored only in automation. UI test support reads `LIFE_ORGANIZE_SCENARIO_ARTIFACTS_DIR` to pass a scenario-artifacts launch argument and `LIFE_ORGANIZE_SCENARIO_SCREENSHOTS_DIR` to copy attached screenshots to a caller-provided directory.

The shared Xcode scheme is `LifeOrganize`. It includes the app target, `LifeOrganizeTests`, and `LifeOrganizeUITests`. The project has Debug and Release configurations and an iOS deployment target of `17.0`. `Scripts/verify-ios.sh` runs `Scripts/ios_static_layout_guard.py`, then runs the Xcode test suite with coverage enabled, then runs `Scripts/ios_coverage_gate.py` unless `IOS_SKIP_COVERAGE_GATE=1`.

`Scripts/screenshots/run-screenshot-tests.sh` runs selected screenshot methods on `LifeOrganizeUITests/LifeOrganizeScenarioUITests`, including first launch, timeline, Things, Carry Forward, search, review queue, Settings, and heavy timeline captures. It extracts `screenshot__*` PNG attachments and compares or updates baselines. `fastlane/Fastfile` exposes only `screenshots` and `update_screenshots` lanes that call that script.
