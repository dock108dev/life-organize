...
`AppRootView` chooses a compact `TabView` shell or a regular-width `NavigationSplitView` shell from the horizontal size class. Compact width keeps Timeline, Things, and Carry Forward as tabs and opens Settings, ledger search, and the review queue as sheets from toolbar buttons. Regular width exposes Timeline, Things, Carry Forward, Search, Review, and Settings in the sidebar, with Review omitted when no ambient review item is visible. `AppRootView` injects `AppSessionState` and `DeveloperModeState`. On launch and whenever the scene becomes active, it runs `ExtractionRecoveryMaintenanceService.repairInterruptedEntries()`, `DerivedFieldMaintenanceService.repairAll()`, and `LedgerReviewItemGenerationService.refresh()`. If this maintenance fails, the app shows an alert that cached ledger fields could not be refreshed.
